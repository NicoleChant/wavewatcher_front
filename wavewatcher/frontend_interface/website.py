...
async def get_requests(iterations : int = 2 , images : int = 1):
...
async def main():
    ...
    if patos_button:
        ...
        if len(df) > 100:
            ...
            df.iloc[-1] = new_data
            df.to_csv(os.path.join(os.path.dirname(__file__), "wave_data.csv"),
                      index=False,
                      mode="w+")
        else:
            pd.DataFrame(data , index=[0]).to_csv(
                os.path.join(os.path.dirname(__file__) , "wave_data.csv"),
                                                    index=False,
                                                    header=None,
                                                    mode="a+")
